...
export async function POST(req) {
  try {
    ...
    // Simpan jawaban tanpa simpan score di tiap jawaban
    for (const item of jawaban) {
      const id = uuidv4();
      await db.query(
        "INSERT INTO jawaban_user (id, user_id, soal_id, kelas_id, jenis, jawaban, benar) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [id, user_id, item.soal_id, kelas_id, 'postest', item.jawaban, item.benar]
      );
    }

    // Update progress posttest_done
    await db.query(
      `INSERT INTO user_kelas_progres (user_id, kelas_id, pretest_done, materi_done, posttest_done, survey_done)
       VALUES (?, ?, 0, 0, 1, 0)
       ON DUPLICATE KEY UPDATE posttest_done = 1`,
      [user_id, kelas_id]
    );

    // Simpan skor total ke tabel nilai (buat tabel nilai kalau belum ada)
    await db.query(
